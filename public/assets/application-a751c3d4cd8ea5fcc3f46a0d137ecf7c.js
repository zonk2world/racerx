...
(function() {
  ...
  PaymentForm = (function() {
    ...
    PaymentForm.prototype.handleStripeResponse = function(status, response) {
      var token;
      if (status === 200) {
        token = response.id;
        $("#payment-form").append($("<input type=\"hidden\" name=\"stripeToken\" />").val(token));
        return $("#payment-form")[0].submit();
      } else {
        $('.payment-errors').html("<div class='alert alert-danger alert-dismissable'> <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button> " + response.error.message + " </div>");
        return $('button#payment-submit').attr('disabled', false);
      }
    };
...
(function() {
  ...
  CustomSeries = (function() {
    function CustomSeries() {
      this.toggleAddUserForm = bind(this.toggleAddUserForm, this);
      this.clickHandler = bind(this.clickHandler, this);
      $('.add_user_form').hide();
      $('.add_user').on('click', this.clickHandler);
      if ($('.alert-danger').length > 0) {
        $('.add-series').click();
      }
    }

    CustomSeries.prototype.clickHandler = function(event) {
      var inviteUserButton, seriesID;
      inviteUserButton = $(event.target);
      seriesID = inviteUserButton.data('custom-series');
      return this.toggleAddUserForm(seriesID);
    };

    CustomSeries.prototype.toggleAddUserForm = function(seriesID) {
      return $(".add_user_form[data-custom-series='" + seriesID + "']").toggle();
    };
  ...
  jQuery(function() {
    if ($('.new_custom_series').length > 0) {
      return new CustomSeries();
    }
  });
...
(function() {
  $(function() {
    if ($('#user_agree_to_terms').length > 0) {
      ...
      return $('#user_agree_to_terms').click(function() {
        if ($('#user_agree_to_terms').is(":checked")) {
          return $('#sign_up_button').attr("disabled", false);
        } else {
          return $('#sign_up_button').attr("disabled", "disabled");
        }
      });
    }
  });
...
(function() {
  $(function() {
    return $('ul.navbarNav li a').on('click', function(event) {
      return $('.tab-pane.active .navbarToggle').click();
    });
  });
...
(function() {
  $(function() {
    return $('#series_join').on('click', function(event) {
...
(function() {
  this.SortRiders = (function() {
    function SortRiders() {
      $('.select_riders').sortable({
        ...
        stop: function(event, ui) {
          return $(ui.item).removeClass("dragging");
        },
        update: function() {
          var payload;
          payload = {
          ...
          return $.post($(this).data('update-url'), payload);
        }
      });
    }
  ...
  jQuery(function() {
    if ($('.select_riders').length > 0) {
      return new SortRiders();
    }
  });
...
googletag.cmd.push(function() {
googletag.defineSlot('/1060327/RXOFantasyLeaderboard', [728,90], 'leaderboard').addService(googletag.pubads());
googletag.defineSlot('/1060327/RXOROSHalfpage', [300,600], 'halfpage').addService(googletag.pubads());
googletag.defineSlot('/1060327/RXOFantasyMobile', [320,50], 'mobile').addService(googletag.pubads());
googletag.defineSlot('/1060327/RXOFantasyLeftSkin', [450,776], 'ss_left').addService(googletag.pubads());
googletag.defineSlot('/1060327/RXOFantasyRightSkin', [450,775], 'ss_right').addService(googletag.pubads());
googletag.defineSlot('/1060327/RXOHomepage3-1Rectangle', [300,100], 'ad_3_1').addService(googletag.pubads());
